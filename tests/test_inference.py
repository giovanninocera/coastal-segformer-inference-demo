...
from coastal_segformer_inference.inference import run_inference
...
class TestInference(unittest.TestCase):
    def test_run_inference(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            configs.mkdir()
            path = configs / "inference.yml"
            path.write_text("output_dir: outputs/inference\nwidth: 64\nheight: 64\ntile_size: 32\nstride: 32\n", encoding="utf-8")
            result = run_inference(load_config(path))
